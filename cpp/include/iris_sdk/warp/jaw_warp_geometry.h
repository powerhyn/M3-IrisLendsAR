 ...
 *   - **홍채/눈 영역 변위 최소화**: 워프는 렌즈 합성보다 앞 단계라 홍채 높이 픽셀이
 *     움직이면 홍채와 렌즈가 어긋난다. **홍채 중심(468/473) 변위 < 최대 변위의 5%
 *     (하드 게이트)**, 눈꼬리(33/263)는 < 12% 로 완화 — 눈꼬리는 절대 px 자체가
 *     미미하고 렌즈가 얹히지 않아 정합 영향이 작다. 최종 판정은 실기기 렌즈 정합.
 *     이 게이트는 kUpperTaper(상방 0.05)·kInteriorTaper[0](볼 중앙 0.08, 게이트 튜닝값)로 지킨다.
