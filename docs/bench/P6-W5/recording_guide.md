...
**기본: 실시간 토글 체감** — 실기기에서 SKU별로 A/B/C/D 버튼을 토글하며 즉석 비교, `ratings_template.csv`에 take당 판정 기록 (`ratings_legend.md` 토글 비교 방법 참조). 녹화 없이 사전 준비 1~5번(특히 native active 확인)만 거치고 바로 토글 평가하면 됨.

**아래 "촬영/후편집/무작위화"는 선택** — 판단이 애매한 take를 self-blind로 재검증하거나, 비교용 영상을 남길 때만 사용.
