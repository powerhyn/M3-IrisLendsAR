...
| 위치 | 현재 상태 |
|---|---|
| `cpp/src/gpu/eye_render_packet_adapter.cpp:89` | `packet.visibility = clamp01(result.confidence * (1 - eyelid_ratio))` — **detector·주입 경로 공유**. `render_confidence = visibility`(:91). |
...
- 권고: **C 우선 검토, 비용 과하면 A**. 어느 쪽이든 **detector 경로 골든 ε 불변**이 절대 제약.
...
3. **주입 렌더 검증(핵심)**: `test_landmark_injection`에 "주입 결과 visibility>0" 케이스 추가, 또는 데모 실기기에서 **TASKS 모드가 (confidence 우회 제거 후에도) 렌즈를 그리는지** 확인. ⚠️ 데모 `TasksToIrisResult.kt:122`의 `confidence=1.0` 우회는 코어 수정 후 **제거**해야 진짜 검증(우회가 남으면 코어 버그가 가려짐).
4. `./gradlew :demo-app:assembleDebug` + 실기기 fsync ON에서 렌즈 정합 유지.
