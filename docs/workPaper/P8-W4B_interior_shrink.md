...
- [ ] **(미결 과제) 내부 taper 미학 비율 판정 — 다수 의견 필요** (사용자 결정 2026-07-15): 현재 값 {볼중앙 0.08, 볼하부 0.35, 입꼬리 0.30, 콧볼 0.20}은 기하 기반 1차 추정. 뷰티 비율은 취향이 갈리는 영역이라 1인 체감으로 확정하지 않고 **여러 사람 의견 수집 후 판정**(기존 렌즈 벤치의 1인 토글 방식 예외). 제약: 볼중앙은 홍채 <5% 게이트 레버라 인상 불가 — 재배분으로 풀 것

### 수집 도구 (벤치 임시, 2026-07-15 구축)

데모 "Taper" 순환 버튼 4프리셋 — `kInteriorTaperPresets` (`jaw_warp_geometry.h`), 벤치 API `iris_sdk_set_interior_taper_preset` (bench_toggles.h, sdk 레벨 atomic이라 GL 재생성 무관). **판정 후 승자를 kInteriorTaper로 고정하고 테이블·API·버튼 전부 제거.**

| # | 라벨 | 볼중앙 | 볼하부 | 입꼬리 | 콧볼 | 게이트(홍채 468, 풀강도) |
|---|------|--------|--------|--------|------|--------------------------|
| 0 | 기본 | 0.08 | 0.35 | 0.30 | 0.20 | 4.73% ✅ |
| 1 | 볼강조 | 0.08 | 0.45 | 0.25 | 0.15 | 4.85% ✅ (여유 0.15%p — 주의) |
| 2 | 입코강조 | 0.08 | 0.28 | 0.38 | 0.28 | 4.77% ✅ |
| 3 | 약하게 | 0.08 | 0.25 | 0.25 | 0.18 | 4.43% ✅ |

테스트 13/13 (프리셋0=기본 비트동일, 범위 밖 클램프, 전 프리셋 풀강도 게이트 루프 포함)
