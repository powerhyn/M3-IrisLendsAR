...
- [ ] **(미결 과제) 내부 taper 미학 비율 판정 — 다수 의견 필요** (사용자 결정 2026-07-15): 현재 값 {볼중앙 0.08, 볼하부 0.35, 입꼬리 0.30, 콧볼 0.20}은 기하 기반 1차 추정. 뷰티 비율은 취향이 갈리는 영역이라 1인 체감으로 확정하지 않고 **여러 사람 의견 수집 후 판정**(기존 렌즈 벤치의 1인 토글 방식 예외). 수집 방법·인원은 추후 결정. 제약: 볼중앙은 홍채 <5% 게이트 레버라 인상 불가 — 재배분으로 풀 것
