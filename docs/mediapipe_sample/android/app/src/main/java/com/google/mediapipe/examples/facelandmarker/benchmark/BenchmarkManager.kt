/**
 * MediaPipe Face Landmarker - Benchmark Manager
 *
 * 성능 및 메모리 벤치마크 통합 관리
 * - 자동 샘플링
 * - 리포트 생성
...
class BenchmarkManager(private val context: Context) {
    ...
    private var modeTag = "cpu"  // "cpu" or "gpu"
    ...
    fun start(durationMs: Long, callback: BenchmarkCallback) {
        ...
        Log.i(TAG, "Benchmark started: duration=${durationMs}ms, mode=$modeTag")
    ...
    fun setGpuEnabled(enabled: Boolean) {
        ...
        modeTag = if (enabled) "gpu" else "cpu"
    }
     ...
     * CSV 파일로 내보내기
     *
     * 파일명 형식: benchmark_mediapipe_[mode]_[timestamp].csv
     */
    fun exportToCsv(): File? {
        if (performanceSamples.isEmpty()) return null

        val dateFormat = SimpleDateFormat("yyyyMMdd_HHmmss", Locale.getDefault())
        val fileName = "benchmark_mediapipe_${modeTag}_${dateFormat.format(Date())}.csv"
    ...
    fun generateMarkdownReport(): String {
        ...
        return buildString {
            appendLine("# MediaPipe Face Landmarker Performance Report")
            appendLine()
            appendLine("## Test Environment")
            appendLine("- **Date**: ${dateFormat.format(Date())}")
            appendLine("- **SDK**: MediaPipe Tasks Vision 0.10.29")
            ...
            appendLine("- **Delegate**: ${if (gpuEnabled) "GPU" else "CPU"}")
            appendLine()
            ...
            if (passCount == 3) {
                ...
                if (stats.fps < 30) appendLine("  - FPS below target")
                if (stats.avgLatencyMs > 33) appendLine("  - Latency above target")
                if (memInfo.totalPssMB > 100) appendLine("  - Memory above target")
            }
        }
    }
